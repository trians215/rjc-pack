...
const toAliasList = () =>
  Object.entries(PRESETS).sort((a,b)=>a[1].localeCompare(b[1]))
...
function buildHashtags(game,{mixPool=true}={}){
  const gNoSpace = String(game).replace(/\s+/g,'');
  const gameSpecific = [`#${gNoSpace}`, `#${gNoSpace}Tergacor`];
  const pickedCore = take(rajacuanCoreHashtags, rand(3,5));
  const pickedPool = mixPool ? take(hashtagsPool, rand(4,7)) : [];
  const all = shuffle([...pickedCore, ...gameSpecific, ...pickedPool]);
  return all.slice(0, rand(7,10));
}
...
function buildDescription({ game, date, link, forceHybrid=false }){
...
function generatePack({game,date,link,mixHashtag,forceHybrid}){
  const title = pick(titleVariants).replace('{GAME}',game).replace('{DATE}',date);
  const desc = buildDescription({game,date,link,forceHybrid});
  const hashtags = buildHashtags(game,{mixPool:mixHashtag});
...
el('#btnList').onclick = ()=>{
  ...
  aliasBox.innerHTML = `<h3 style="margin-top:0">Alias Game</h3><div class="mono">${toAliasList()}</div>`;
};
...
el('#btnExport').onclick = ()=>{
  ...
  const a = document.createElement('a'); a.href = url; a.download = 'rajacuan-youtube.txt'; a.click(); URL.revokeObjectURL(url);
};

// === REPLACE seluruh blok ini di generator.js ===
...
${pack.tags.join(', ')}`;
    ...
    // === SINGLE VIEW (tanpa duplikat) ===
    const elCard = document.createElement('div');
    elCard.className = 'item';
    elCard.innerHTML = `
    ...
    // tombol copy per-bagian
    elCard.querySelectorAll('[data-copy]').forEach(btn => {
      btn.addEventListener('click', () => {
        let text = '';
        const type = btn.getAttribute('data-copy');
        if (type === 'title') text = elCard.querySelector(`#title-${i}`).textContent;
        else if (type === 'desc') text = elCard.querySelector(`#desc-${i}`).textContent;
        else if (type === 'hash') text = elCard.querySelector(`#hash-${i}`).textContent;
        else if (type === 'tags') text = elCard.querySelector(`#tags-${i}`).textContent;
    ...
    outEl.appendChild(elCard);
    blocks.push(copyAllText); // untuk Copy All global/export
  }

  // simpan semua output untuk tombol Copy All/Export global
