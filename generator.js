...
function buildHashtags(game,mix=true){ const g=game.replace(/\s+/g,''); const gameTags=[`#${g}`,`#${g}Tergacor`]; const pickedCore=take(rjCore,randInt(3,5)); const pickedPool=mix?take(hashtagsPool,randInt(5,7)):[]; return shuffle([...pickedCore,...gameTags,...pickedPool]).slice(0,randInt(6,10)); }
